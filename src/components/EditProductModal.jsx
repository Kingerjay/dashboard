...
const EditProductModal = ({ isOpen, onClose, onSubmit, product }) => {
  ...
  // When product changes or modal opens/closes, update the form
  useEffect(() => {
    if (product && isOpen) {
      // Extract numeric price value (remove "Php" and trim)
      const priceValue = product.price ? 
        product.price.replace('Php', '').trim() : '';
      ...
      // Reset form when opening with no product
      setFormData({
        widget: null,
        brand: '',
        category: '',
  ...
  const handleChange = (e) => {
    ...
    if (name === 'brand') {
      setFormData(prev => ({
        ...
        productName: '',
        price: '',
        sku: product ? prev.sku : '' // Keep existing SKU when editing
      }));
      ...
      setFormData(prev => ({
        ...
        sku: product ? prev.sku : prev.sku || generateSKU() // Generate new SKU
      }));
    } else {
      setFormData(prev => ({
        ...prev,
  ...
  const handleSubmit = (e) => {
    ...
    const submissionData = {
      ...
      originalImage: product ? product.image : null // Keep original image if not changed
    };
    ...
    <div className="fixed top-0 left-[270px] right-0 bottom-0 bg-[rgba(25,26,25,1)] z-50">
      ...
      <div className="w-full h-full overflow-y-auto p-10">
        <form onSubmit={handleSubmit} className="max-w-4xl my-0 mx-auto space-y-6">
          <h2 className="text-xl text-white font-medium mb-8">Edit Product</h2>
